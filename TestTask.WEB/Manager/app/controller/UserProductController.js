Ext.define('UM.controller.UserProductController', {
    ...
    views: ['product.userList', 'order.Make', 'product.Basket', 'product.AddToBasket', 'product.EditBasket'],
    ...
    //createOrder: function (view, cell, rowIndex, colIndex, e, record, row) {
    //    var create = Ext.create('UM.view.order.Make').show();
    //    create.down('form').loadRecord(record);
    //},

    //makeOrder: function (button) {
    //    var win = button.up('window'),
    //        form = win.down('form'),
    //        row = form.getRecord(),
    //        values = form.getValues();
    ...
    //    var record = Ext.create("UM.model.Order");
    //    var orderNumber = Ext.id();
    //    orderNumber = parseInt(orderNumber.replace("ext-gen", ""));

    //    record.set('id', orderId);
    //    record.set('ORDER_NUMBER', orderNumber);
    //    record.set('status', 'New');
    //    order_store.add(record);
    ...
    //    var rec = Ext.create("UM.model.OrderItem");
    //    var orderItemId = ((((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1) +
    //        (((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1) + '-' +
    //        (((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1) + '-4' +
    //        (((1 + Math.random()) * 0x10000) | 0).toString(16).substring(1).substr(0, 3) + '-' +
    ...
    //    rec.set('id', orderItemId);
    //    rec.set('PRODUCT_COUNTS', values.PRODUCT_COUNTS);
    //    rec.set('OrderID', orderId);
    //    rec.set('ProductID', values.id);
    //    rec.set('PRICE', values.PRICE)
        
    //    orderItemStore.add(rec);
